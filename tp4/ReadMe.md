...
```
```sql
grant dev to devsecops1,devsecops2;
```
...
```sql
revoke connect ,create session,select any table from tester;
```
...
grant select on emp to devsecops;
```
...
```sql
revoke select on emp from devsecops;
```
